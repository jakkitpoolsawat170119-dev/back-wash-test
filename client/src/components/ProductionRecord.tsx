...
const ProductionRecord: React.FC<ProductionRecordProps> = ({ operatorName, onBack, onHome }) => {
  ...
  const handleCookingBatchChange = (lineId: number, selectedBatch: string) => {
    ...
    const lastBatch = line.history.length > 0 
      ? line.history[line.history.length - 1].batch 
      : line.shiftBatch;

    if (!lastBatch) {
      alert("กรุณาเลือก 'รับช่วงต่อจาก Batch' ก่อนครับ");
      return;
    }

    const expectedBatch = getNextBatch(lastBatch);
    if (selectedBatch !== expectedBatch) {
      alert(`ลำดับ Batch ไม่ถูกต้อง! ลำดับที่ต้องทำคือ Batch ${expectedBatch}`);
      return;
    }
  ...
  const handleStart = (lineId: number) => {
    const line = lines[lineId];
    if (!line.flavor || !line.cookingBatch) {
      alert("กรุณาเลือก รสชาติ และ Batch เริ่มต้ม ก่อนกด Start");
      return;
    }
    const now = new Date();
    ...
    setLines(prev => ({
      ...prev,
      [lineId]: { ...prev[lineId], startTime: timeStr, startRaw: now, doneTime: null, isProcessing: true }
    }));
  };
  ...
  const handleDone = async (lineId: number) => {
    ...
    const newCompletedBatch: CompletedBatch = {
      line: lineId,
      batch: line.cookingBatch,
      flavor: line.flavor,
    ...
    try {
      await fetch(`${apiUrl}/api/production/log`, {
        method: 'POST',
  ...
  return (
    <div style={{ paddingBottom: '50px' }}>
      <h2 className={styles.header} style={{
        width: '95%', maxWidth: '500px', margin: '20px auto 10px auto',
        background: 'linear-gradient(135deg, #4caf50, #2e7d32)', borderRadius: '15px',
        padding: '15px', color: '#ffffff', textAlign: 'center', boxShadow: '0 6px 15px rgba(76, 175, 80, 0.3)'
      }}>
        Production Control
      </h2>

      <div className={styles.navHeader} style={{ flexWrap: 'wrap', gap: '10px', marginTop: '0', marginBottom: '20px', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button className={styles.backBtn} onClick={onBack} style={{ margin: 0, padding: '8px 15px', fontSize: '0.8rem', background: '#2e7d32', color: 'white', borderRadius: '8px' }}>
            🚪 ออกจากระบบการผลิต
          </button>
        </div>
        <div style={{ fontWeight: 'bold', color: '#2e7d32' }}>👤 {operatorName}</div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px', padding: '10px' }}>
        {[1, 2, 3, 4].map(lineId => {
          ...
          return (
            <div key={lineId} className={styles.stepCard} style={{ 
              borderColor: '#4caf50', 
              background: line.showInputs ? '#ffffff' : '#f1f8e9', 
              padding: '20px'
            }}>
                ...
                <>
                  <div className={styles.formGroup} style={{ marginBottom: '10px' }}>
                    ...
                    <select 
                      ...
                      value={line.flavor} 
                      onChange={(e) => setLines(prev => ({ ...prev, [lineId]: { ...prev[lineId], flavor: e.target.value } }))}
                  ...
                  <div style={{ 
                    ...
                    gap: '8px', 
                    marginBottom: '20px', 
                    background: 'linear-gradient(90deg, #fff9c4 0%, #e3f2fd 50%, #e8f5e9 100%)', 
                    padding: '12px', 
                    borderRadius: '18px', 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px' }}>
                    <button 
                      onClick={() => handleStart(lineId)}
                      disabled={line.isProcessing || !line.cookingBatch}
                      style={{ flex: 1.5, padding: '12px', background: line.isProcessing ? '#ccc' : '#4caf50', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', cursor: 'pointer' }}
                    >
                      ▶️ Start Batch {line.cookingBatch}
                    </button>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                        <thead>
                            <tr style={{ backgroundColor: '#f5f5f5' }}>
                                ...
                                <th style={{ padding: '10px', border: '1px solid #ddd' }}>รสชาติ</th>
                                <th style={{ padding: '10px', border: '1px solid #ddd' }}>เวลา Start-Done</th>
                                <th style={{ padding: '10px', border: '1px solid #ddd' }}>Brix</th>
                                <th style={{ padding: '10px', border: '1px solid #ddd' }}>PH</th>
                                <th style={{ padding: '10px', border: '1px solid #ddd' }}>รวมเวลา</th>
                            </tr>
                        </thead>
                        <tbody>
                            {allHistory.length > 0 ? allHistory.map((h, i) => (
                                <tr key={i} style={{ textAlign: 'center' }}>
                                    <td style={{ padding: '10px', border: '1px solid #ddd' }}>Line {h.line}</td>
                                    <td style={{ padding: '10px', border: '1px solid #ddd', fontWeight: 'bold' }}>{h.batch}</td>
                                    <td style={{ padding: '10px', border: '1px solid #ddd' }}>{h.flavor}</td>
                                    <td style={{ padding: '10px', border: '1px solid #ddd' }}>{h.startTime} - {h.doneTime}</td>
                                    <td style={{ padding: '10px', border: '1px solid #ddd', color: '#1b5e20', fontWeight: 'bold' }}>{h.brix}</td>
                                    <td style={{ padding: '10px', border: '1px solid #ddd', color: '#1b5e20', fontWeight: 'bold' }}>{h.ph}</td>
